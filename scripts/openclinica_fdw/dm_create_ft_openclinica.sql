...
$b$
/* Create a local foreign table for each table in the foreign schema. */
DECLARE
  r RECORD;
BEGIN
  FOR r IN
      ...
      array_to_string(
        array_agg(
          concat_ws(
            ...
            format_type(ft_pg_attribute.atttypid, ft_pg_attribute.atttypmod))
        ORDER BY ft_pg_attribute.attnum),
        $$, $$) AS table_def
    FROM openclinica_fdw.ft_pg_attribute
    LEFT JOIN openclinica_fdw.ft_pg_class
      ON ft_pg_class.oid = ft_pg_attribute.attrelid
    LEFT JOIN openclinica_fdw.ft_pg_namespace
  ...
  SELECT
    format(
      $$ CREATE FOREIGN TABLE openclinica_fdw.%1$I (%2$s)
           SERVER openclinica_fdw_server
           OPTIONS (schema_name %3$L,
