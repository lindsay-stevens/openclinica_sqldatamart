CREATE VIEW dm.clinicaldata AS
  WITH multi_split AS (
    SELECT
      id.item_data_id,
    ...
    FROM openclinica_fdw.audit_log_event AS ale
    WHERE
  ...
  INNER JOIN openclinica_fdw.subject_event_status AS ses
  ...
  INNER JOIN openclinica_fdw.status AS ec_s
  ...
  LEFT JOIN openclinica_fdw.user_account AS ua_se_o
    ON ua_se_o.user_id = se.owner_id
  LEFT JOIN openclinica_fdw.user_account AS ua_se_u
    ON ua_se_u.user_id = se.update_id
  LEFT JOIN openclinica_fdw.user_account AS ua_ec_o
    ON ua_ec_o.user_id = ec.owner_id
  LEFT JOIN openclinica_fdw.user_account AS ua_ec_u
    ON ua_ec_u.user_id = ec.update_id
  LEFT JOIN openclinica_fdw.user_account AS ua_ec_v
    ON ua_ec_v.user_id = ec.validator_id
  LEFT JOIN openclinica_fdw.user_account AS ua_ec_s
    ON ua_ec_s.user_id = ec.sdv_update_id
  LEFT JOIN openclinica_fdw.user_account AS ua_id_o
    ON ua_id_o.user_id = id.owner_id
  LEFT JOIN openclinica_fdw.user_account AS ua_id_u
  ...
  WHERE
    ...
    AND
    CASE WHEN
      CASE WHEN edc.parent_id IS NOT NULL
        THEN
          edc.event_definition_crf_id =
          (
            SELECT max(edc_max.event_definition_crf_id) edc_max
            FROM openclinica_fdw.event_definition_crf AS edc_max
            WHERE
              edc_max.study_event_definition_id = se.study_event_definition_id
              AND edc_max.crf_id = crf.crf_id
            GROUP BY
    ...
    ELSE
      CASE WHEN edc.parent_id IS NULL AND
                (
                  SELECT count(edc_count.event_definition_crf_id) edc_count
                  FROM openclinica_fdw.event_definition_crf AS edc_count
                  WHERE
        ...
        THEN
          edc.event_definition_crf_id =
          (
            SELECT min(edc_min.event_definition_crf_id) edc_min
            FROM openclinica_fdw.event_definition_crf AS edc_min
            WHERE
              edc_min.study_event_definition_id = se.study_event_definition_id
